...
while ($db->next_record(MYSQL_ASSOC)) {
    $fieldvalue = \MyAdmin\App::decryptOld($db->Record['fieldvalue']);
    if ($fieldvalue != $db->Record['fieldvalue']) {
        $query = "update swcustomfieldvalues set fieldvalue='".$db2->real_escape(\MyAdmin\App::encrypt($fieldvalue))."' where customfieldvalueid={$db->Record['customfieldvalueid']}";
        echo "$query\n";
        $db2->query($query);
    }
}
